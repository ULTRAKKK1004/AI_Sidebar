...
from PySide6.QtWidgets import (
    ...
    QGroupBox, QTabBar, QCheckBox, QComboBox, QPlainTextEdit, QTextBrowser # Added TextBrowser
)
from PySide6.QtCore import (
    ...
    QMetaObject, Q_ARG # Added for cross-thread signals
)
                            ...
                            try:
                                ...
                                if "choices" in data and data["choices"]:
                                    ...
                                    if message_content:
                                        ...
                                        logging.info(f"Stream chunk: {message_content}")
...
class AssistantWidget(QWidget):
    ...
    def send_prompt(self):
        ...
        if prompt_text:
            # Add user message to UI with markdown formatting
            user_html = f"""
            <div style='margin: 10px 0; padding: 10px; background-color: #e9ecef; border-radius: 4px;'>
                <strong style='color: #0d6efd;'>You:</strong> {prompt_text}
            </div>
            """
            self.append_html_to_view(user_html)
    ...
    def append_html_to_view(self, html):
        """Appends HTML content to the conversation view."""
        self.conversation_view.moveCursor(QTextCursor.End)
        self.conversation_view.insertHtml(html)
        self.conversation_view.moveCursor(QTextCursor.End)

    @Slot(str)
    def handle_stream_chunk(self, chunk):
        """Appends a chunk of the assistant's response to the view and buffer."""
        # Initialize response HTML if this is the first chunk
        if not hasattr(self, '_current_response_html'):
            self._current_response_html = """
            <div style='margin: 10px 0; padding: 10px; background-color: #f8f9fa; border-radius: 4px;'>
                <strong style='color: #198754;'>Assistant:</strong> 
            """
            self.append_html_to_view(self._current_response_html)
            self._current_response_html = ""
        ...
        if '```' in chunk:
            ...
            for i, part in enumerate(parts):
                if i % 2 == 0:  # Regular text
                    self.append_html_to_view(part)
                else:  # Code block
                    code_html = f'<pre style="background-color: #f1f1f1; padding: 10px; border-radius: 4px; overflow-x: auto;">{part}</pre>'
                    self.append_html_to_view(code_html)
        else:
            self.append_html_to_view(chunk)
    ...
    def clear_conversation(self):
        ...
        if hasattr(self, '_current_response_html'):
            delattr(self, '_current_response_html')
    ...
    def on_stream_finished(self):
        ...
        self.send_button.setEnabled(True)
        self.new_chat_button.setEnabled(True)
        self.append_html_to_view("<br>") # Add newline after response
        if hasattr(self, '_current_response_html'):
            delattr(self, '_current_response_html')
...
class MainWindow(QMainWindow):
    ...
    def create_tray_icon(self):
        ...
        tray_menu.addAction(self.always_on_top_action); tray_menu.addSeparator()
...
if __name__ == "__main__":
    ...
    try:
        window = MainWindow()
        # window.show() # Don't show initially, let tray icon handle it
