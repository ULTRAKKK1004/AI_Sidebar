...
default_webui_endpoint = "https://www.ai-personalserv.com"    
default_webui_apikey = "f3a77466e9e6842b1"
default_webui_model = "gpt-4o-mini"
default_tavily_apikey = "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"
Proxy_http = "http://168.219.61.252:8080"
Proxy_https = "http://168.219.61.252:8080"
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, settings, parent=None):
        ...
        self.webui_endpoint_edit.setPlaceholderText("")
        ...
        self.webui_apikey_edit.setPlaceholderText("")
        webui_layout.addRow("API Key:", self.webui_apikey_edit)
        ...
        # Web Search Settings
        search_group = QGroupBox("Web Search API")
        search_layout = QFormLayout(search_group)
        
        # Search API 선택 콤보박스
        self.search_api_combo = QComboBox()
        self.search_api_combo.addItems(["Tavily", "Google"])
        self.search_api_combo.currentTextChanged.connect(self.toggle_search_api_fields)
        search_layout.addRow("Search API:", self.search_api_combo)
        
        # Google Search 필드
        self.google_api_key_edit = QLineEdit()
        self.google_api_key_edit.setEchoMode(QLineEdit.Password)
        search_layout.addRow("Google API Key:", self.google_api_key_edit)
        
        self.google_cx_edit = QLineEdit()
        search_layout.addRow("Google Custom Search ID (CX):", self.google_cx_edit)
        
        # Tavily Search 필드
        self.tavily_api_key_edit = QLineEdit()
        self.tavily_api_key_edit.setEchoMode(QLineEdit.Password)
        self.tavily_api_key_edit.setPlaceholderText("")
        search_layout.addRow("Tavily API Key:", self.tavily_api_key_edit)
    ...
    def toggle_search_api_fields(self, selected_api):
        # 선택된 검색 API에 따라 필드 활성화/비활성화
        if selected_api == "Google":
            self.google_api_key_edit.setEnabled(True)
            self.google_cx_edit.setEnabled(True)
            self.tavily_api_key_edit.setEnabled(False)
        else:  # Tavily
            self.google_api_key_edit.setEnabled(False)
            self.google_cx_edit.setEnabled(False)
            self.tavily_api_key_edit.setEnabled(True)
    ...
    def load_settings_values(self):
        ...
        # WebUI 설정
        # 기본값을 지정하여 빈 값일 경우에도 사용 가능하도록 변경
        ...
        self.tavily_api_key_edit.setText(self.settings.value("tavily/apikey", defaultValue=""))
        
        # 검색 API 선택 로드
        search_api = self.settings.value("search/selected_api", defaultValue="Tavily")
        self.search_api_combo.setCurrentText(search_api)
        self.toggle_search_api_fields(search_api)
        
        # 모델 정보 로드
        saved_models = self.settings.value("webui/available_models", defaultValue=[])
        selected_model = self.settings.value("webui/selected_model", defaultValue="gpt-4o-mini")
        ...
        if saved_models:
            ...
            if selected_model in model_names:
                ...
                # 선택된 모델이 목록에 없으면 추가
                self.webui_model_combo.addItem(selected_model)
                self.webui_model_combo.setCurrentText(selected_model)
        else:
            # 기본 모델 추가
            self.webui_model_combo.addItem("gpt-4o-mini")
            self.webui_model_combo.setCurrentText("gpt-4o-mini")
            
            if self.webui_endpoint_edit.text():
        ...
        if endpoint:
            try:
                ...
                if models:
                    ...
                    # 모델을 가져오지 못했을 때 기본 모델 설정
                    self.webui_model_combo.clear()
                    self.webui_model_combo.addItem("gpt-4o-mini")
                    self.webui_model_combo.setCurrentText("gpt-4o-mini")
                    QMessageBox.warning(self, "No Models", "No models found at the specified endpoint. Using default model.")
                    self.settings.setValue("webui/available_models", [{"id": "gpt-4o-mini", "name": "gpt-4o-mini"}])
                ...
                self.webui_model_combo.addItem("gpt-4o-mini")
                self.webui_model_combo.setCurrentText("gpt-4o-mini")
                QMessageBox.warning(self, "Fetch Error", f"Failed to fetch models. Using default model.\nError: {e}")
                self.settings.setValue("webui/available_models", [{"id": "gpt-4o-mini", "name": "gpt-4o-mini"}])
            except json.JSONDecodeError:
                ...
                self.webui_model_combo.addItem("gpt-4o-mini")
                self.webui_model_combo.setCurrentText("gpt-4o-mini")
                QMessageBox.warning(self, "Fetch Error", "Received invalid response from the server. Using default model.")
                self.settings.setValue("webui/available_models", [{"id": "gpt-4o-mini", "name": "gpt-4o-mini"}])
            except Exception as e:
                ...
                self.webui_model_combo.addItem("gpt-4o-mini")
                self.webui_model_combo.setCurrentText("gpt-4o-mini")
                QMessageBox.warning(self, "Fetch Error", f"An unexpected error occurred. Using default model.\nError: {e}")
                self.settings.setValue("webui/available_models", [{"id": "gpt-4o-mini", "name": "gpt-4o-mini"}])
    ...
    def save_and_close(self):
        ...
        model = self.webui_model_combo.currentText()
        if not model:
            model = "gpt-4o-mini"
            
        self.settings.setValue("webui/endpoint", endpoint)
        self.settings.setValue("webui/apikey", api_key)
        self.settings.setValue("webui/selected_model", model)
        
        # 프록시 설정 저장
        self.settings.setValue("webui/disable_proxy", self.disable_proxy_checkbox.isChecked())
        
        # 검색 API 선택 저장
        selected_api = self.search_api_combo.currentText()
        self.settings.setValue("search/selected_api", selected_api)
        ...
        self.settings.setValue("tavily/apikey", tavily_api_key)
...
class AssistantWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.google_cx = ""
        self.tavily_api_key = ""
        self.search_api = "Tavily"  # 기본 검색 API
        ...
        self.search_api_combo = QComboBox()
        self.search_api_combo.addItems(["Tavily", "Google"])
        self.search_api_combo.setToolTip("Select search provider")
        ...
        tools_layout.addWidget(self.search_api_combo)
    ...
    def handle_web_search(self):
        ...
        if selected_api == "Google" and (not self.google_api_key or not self.google_cx):
            QMessageBox.warning(self, "API Keys Missing", 
                               "Google API Key and Custom Search Engine ID are required for Google search. Please configure them in settings.")
            return
        
        if selected_api == "Tavily" and not self.tavily_api_key:
            # Tavily API Key가 설정되지 않았을 때 기본값 사용
            self.tavily_api_key = default_tavily_apikey
            
        self.web_search_signal.emit(query)
        self.append_text_to_view(f"\n**Searching the web using {selected_api} for:** {query}\n")
    ...
    def perform_web_search(self, query):
        try:
            ...
            if selected_api == "Google":
                ...
                search_results = search_tavily(query, self.tavily_api_key)
            
            if not search_results:
                self.append_text_to_view("\n**No search results found.**\n")
                return
            
            results_text = f"\n**Web Search Results from {selected_api}:**\n\n"
            ...
            self.conversation_history.append({
                "role": "system",
                "content": f"Web search results from {selected_api} for query '{query}': {json.dumps(search_results)}"
            })
        except Exception as e:
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Google API 키 및 Tavily API 키 설정 전달
        self.assistant_widget.google_api_key = self.google_api_key
        self.assistant_widget.google_cx = self.google_cx
        self.assistant_widget.tavily_api_key = self.tavily_api_key
        self.assistant_widget.search_api = self.search_api
        self.assistant_widget.search_api_combo.setCurrentText(self.search_api)
    ...
    def load_settings(self):
        ...
        # 검색 API 선택 로드
        self.search_api = self.settings.value("search/selected_api", defaultValue="Tavily")
        ...
        # Tavily API 설정 로드
        self.tavily_api_key = self.settings.value("tavily/apikey", defaultValue="")
    ...
    def update_assistant_availability(self):
        ...
        logging.info("Assistant enabled.")
    
    @Slot(list)
    def handle_assistant_conversation(self, messages):
        # WebUI 엔드포인트, API 키, 모델이 비어있으면 기본값 사용
        if not self.webui_endpoint:
            self.webui_endpoint = default_webui_endpoint
            
        if not self.webui_apikey:
            self.webui_apikey = default_webui_apikey
            
        if not self.webui_model:
            self.webui_model = default_webui_model
    ...
    def handle_web_search(self, query):
        # 검색 API에 따라 필요한 키 확인
        if self.search_api == "Google" and (not self.google_api_key or not self.google_cx):
