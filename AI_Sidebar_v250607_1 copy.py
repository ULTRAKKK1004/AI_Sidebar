...
class ClipboardItemWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        if self.clipboard_item.data_type == "text":
            ...
            self.content_label = QLabel(f"{self.index+1}. {preview}")
            self.content_label.setWordWrap(True)
            self.content_label.setMaximumWidth(350)
            self.content_label.setMinimumHeight(60)
        elif self.clipboard_item.data_type == "image":
            self.content_label = QLabel(f"{self.index+1}. [Image]")
            if isinstance(self.clipboard_item.data, QPixmap):
                self.icon_label = QLabel()
                self.icon_label.setPixmap(self.clipboard_item.data.scaled(64, 64, Qt.KeepAspectRatio, Qt.SmoothTransformation))
    ...
    def update_style(self):
        ...
        if self.clipboard_item.hidden:
            if self.content_label:
                self.content_label.setText(f"{self.index+1}. [숨김됨]")
            if self.icon_label:
            ...
            if self.content_label:
                if self.clipboard_item.data_type == "text":
                    preview = self.clipboard_item.data.split("\n")[0][:50] + ("..." if len(self.clipboard_item.data) > 50 else "")
                    self.content_label.setText(f"{self.index+1}. {preview}")
                elif self.clipboard_item.data_type == "image":
                    self.content_label.setText(f"{self.index+1}. [Image]")
            if self.icon_label:
...
class MainWindow(QMainWindow):
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            logging.error(f"Error hiding tray icon: {e}", exc_info=True)
    ...
    def hide_clipboard_item(self, index):
        # 삭제가 아니라 hidden 속성만 True로 변경
        if 0 <= index < len(self.clipboard_history):
            self.clipboard_history[index].hidden = True
            self.update_clipboard_list_ui()
